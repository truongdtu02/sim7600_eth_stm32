...
//###################################################################################################################
uint32_t W25qxx_WritePage_Quick(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToWrite_up_to_PageSize)
{
	...
	W25qxx_WaitBusy(0); //ready to write
	...
	W25qxx_WaitBusy(0); //ready to next write

	return NumByteToWrite_up_to_PageSize;
}
//###################################################################################################################
...
//###################################################################################################################
uint8_t W25qxx_ReadByte_Quick(uint32_t ReadAddr)
{
	uint8_t value;
	...
	W25qxx_Spi((ReadAddr & 0xFF00) >> 8);
	W25qxx_Spi(ReadAddr & 0xFF);
	value = W25qxx_Spi(W25QXX_DUMMY_BYTE);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
...
//###################################################################################################################
int W25qxx_IsSus() { //suspend erasing
	if((int)W25qxx_ReadStatusRegister(2) & (1 << 7)) {
		return true;
	}
	return false;
}
//###################################################################################################################
int W25qxx_IsBusy() {
	if(W25qxx_ReadStatusRegister(1) & 0x1) {
		return true;
	}
	return false;
}
//###################################################################################################################
int64_t W25qxx_WaitBusy_cnt = 0; 
int W25qxx_WaitBusy(uint8_t value){ //wait until Busy == value
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
	W25qxx_Spi(0x05);
	do
	{
		w25qxx.StatusRegister1 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
		W25qxx_WaitBusy_cnt++;
	} while ((w25qxx.StatusRegister1 & 0x01) != value);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
...
//###################################################################################################################
int64_t W25qxx_WriteEnable_QuickWait_cnt = 0;
void W25qxx_WriteEnable_QuickWait(void)
{
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
	W25qxx_Spi(0x06);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
	// W25qxx_Spi(W25QXX_DUMMY_BYTE); //delay

	//read SR1 bit 1 (WEL), wait until WEL == 1
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
	W25qxx_Spi(0x05);
	do
	{
		W25qxx_WriteEnable_QuickWait_cnt++;
		w25qxx.StatusRegister1 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
	} while ((w25qxx.StatusRegister1 & 0x02) == 0);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
	// W25qxx_Spi(W25QXX_DUMMY_BYTE); //delay
}
//###################################################################################################################
